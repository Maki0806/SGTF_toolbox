...
function [x_rho,F_rho] =detail_smoothing_for_graph_signals(g_s,y,sigma_d,sigma_r,rho,A)
...
for i = 1:length(ci)
    temp = [temp;exp(-((A(ci(i),cj(i))))/(sigma_d2))*exp((g_s2(ci(i),cj(i)))/(sigma_r2))];
end

clear g_s
W= sparse([ci,cj],[cj,ci],[temp;temp],N1,N1);
